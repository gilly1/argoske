...
class ContractLogic
{
        ...
        DB::transaction(function () use ($subdomain, $request, $data, $state) {
            ...
            if ($state == 'save') {
                ...
                foreach ($allcontractItems as $allcontractItem) {
                    [$installments, $total_cash_pay, $duration, $intrest_for_one_month, $loading_amount_for_one_month, $principle_for_one_month, $currency] = InquiryCalculatorLogic::price_calculator($allcontractItem->stock_id, $allcontractItem->installment, $allcontractItem->duration, $allcontractItem->quantity);
                ...
                if ($request->status == 2) {
                    $customer = AccountCustomer::where('id', $data->account_customer_id)->first();
                    ...
                    $crmPerson->ref  = $customer->other_names;
                    ...
                    if ($newCrmPerson) {
                        ...
                        $custBranch->debtor_no  = $debtorsMaster->id;
                    ...
                    $salesOder->branch_code = isset($custBranch->branch_code) ? $custBranch->branch_code : $debtorsMaster->custBranch->first()->branch_code;
                    ...
                    foreach ($allcontractItems as $allcontractItem) {
                        [$installments, $total_cash_pay, $duration, $intrest_for_one_month, $loading_amount_for_one_month, $principle_for_one_month, $currency, $unit_price] = InquiryCalculatorLogic::price_calculator($allcontractItem->stock_id, $allcontractItem->installment, $allcontractItem->duration, $allcontractItem->quantity);
                        ...
                        $gl_trans->type_no = $debtor_trans->trans_no; //cust_allocations id
                        $gl_trans->tran_date = $trans_date;
                        $gl_trans->account = 2150; //for -ve and 1060 for +ve
                        $gl_trans->memo_ = ' ';
                        $gl_trans->amount = 0; //also for +ve account
                        $gl_trans->save();

                        self::createSchedule($total_cash_pay, $installments, $intrest_for_one_month, $loading_amount_for_one_month, 0, $data->id, $request->repayment_date, $allData = [], $total_cash_pay, $debtor_trans->trans_no);
    ...
    protected static function createSchedule($amount, $installment, $intrest, $loading, $i, $contract_id, $repayment_date, $allData, $total_cash_pay, $trans_no)
    {
        if ($amount > $installment) {
            ...
            if ($amount > 0) {
                $allData[] = [
                    'principle' => $installment - $intrest - $loading,
                    'trans_no' => $trans_no,
                    'intrest' => $intrest,
                    'loading' => $loading,
            ...
            if ($amount > 0) {
                $allData[] = [
                    ...
                    'trans_no' => $trans_no,
                    'intrest' => $balance < 0 ? '0' : $intrest,
                    'loading' => $balance < 0 ? '0' : $loading,
                    'installment' => $amount,
                    'balance' => 0,
                    'actual_balance' => $total_cash_pay,
                    'contract_items_id' => $contract_id,
