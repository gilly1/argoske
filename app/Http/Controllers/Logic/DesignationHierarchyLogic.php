...
class DesignationHierarchyLogic
{
    const route = 'designation_hierarchies';
    const redirect = 'designation_hierarchies/designation_hierarchies';

    public $model;

    function __construct(
        DesignationHierarchy $model
    ){
        $this->model = $model;
    }
    ...
    {
        return [
            'Hierarchy'=>['relationship','hierarchy','name'],
			'Designation'=>['relationship','designation','name'],
			'Parent designation'=>['relationship','parentDesignation','name'],
    ...
    {    
        ...
        $values = [
            'hierarchy_id'=>
			[
				'select','select','Choose Hierarchy',6,true,true,'hierarchy_id','hierarchy_id','Select hierarchy_id','',Hierarchy::all(),isset($forEdit) ? DesignationHierarchy::with('hierarchy')->where('id',$forEdit->id)->first()->hierarchy : ''
			],
			'designation_id'=>
			[
				'select','select','Choose Designation',6,true,true,'designation_id','designation_id','Select designation_id','',Designation::all(),isset($forEdit) ? DesignationHierarchy::with('designation')->where('id',$forEdit->id)->first()->designation : ''
			],
			'parnt_designation'=>
			[
				'select','select','Parent designation',6,true,true,'parnt_designation','parnt_designation','Select parnt_designation','',Designation::all(),isset($forEdit) ? DesignationHierarchy::with('designation')->where('id',$forEdit->id)->first()->parentDesignation : ''
			],
    ...
    {  
        ...
        $uniqueBy = '';
        $modelName = 'DesignationHierarchy';
        $model = 'App\Model';
        $columns = [
            'hierarchy_id','designation_id','parnt_designation'
    ...
    {     
        ...
        $headings = [ //variable
            'Hierarchy','Designation','Parnt designation'
			//headings
        ];
        $data = $this->model->select('hierarchy_id','designation_id','parnt_designation'
			)->get(); //variable
