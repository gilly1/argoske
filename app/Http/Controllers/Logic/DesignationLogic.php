...
class DesignationLogic
{
    ...
    const redirect = 'designations/designations';
    ...
    public  function save($subdomain,$request,$data,$state)
    {
        ...
        {
            $updateData = $this->model->where('id',$data->id)->first();
            $updateData->update(['designation_id' => $data->id]);
        }
    ...
    public  function tableColumns() //variable
    {
        return [
            'Name'=>'name',
			'Parent designation'=>['relationship','designation','name'],
            'Status' => ['enum','is_approved',null,'Not Approved','Approved'] 
    ...
    {    
        ...
        $values = [
			...
			'designation_id'=>
			[
				'select','select','Choose Parent designation',6,true,true,'designation_id','designation_id','Select designation_id','',Designation::all(),isset($forEdit) ? Designation::with('designation')->where('id',$forEdit->id)->first()->designation : ''
			],
    ...
    {     
        ...
        $data = $this->model->select('name','designation_id'
			)->get(); //variable
