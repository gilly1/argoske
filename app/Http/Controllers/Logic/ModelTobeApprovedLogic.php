...
class ModelTobeApprovedLogic
{
    const route = 'model_tobe_approveds';
    const redirect = 'model_tobe_approveds/model_tobe_approveds';

    public $model;

    function __construct(
        ModelTobeApproved $model
    ){
        $this->model = $model;
    }
    ...
    {    
        ...
        $values = [
			...
			'model'=>
			[
				'text','string','Model',6,true,true,'model','model','Enter Model'
			],
    ...
    {     
        ...
        $data = $this->model->select('name','model'
			)->get(); //variable
