        ...
        <div class="form-group has-feedback col-md-{{$obj->col}}">
            ...
            <div class="input-group date">
                ...
                <input type="text" class="form-control pull-right{{ $errors->has($obj->id) ? ' is-invalid' : '' }}"
                    value="@if( $forEdit  ){{\Carbon\Carbon::parse( $forEdit->{$obj->name} )->format('m/d/Y')}}@else{{old($obj->name)}}@endif"  
            ...
            @foreach($obj->loop as $coll)
                ...
                <?php 
                 ...
                 ?>
                    ...
                    @if($coll->id == $name->id)
                        <option selected value="{{$name->id}}">{{$name->$name_key}} {{isset($name->full_name) ? ' - '. $name->full_name : '' }} {{isset($name->name) && $name->name != $name->$name_key ? ' - '. $name->name : '' }}</option>
                    @else
                        <option value="{{$coll->id}}">{{$coll->$name_key}} {{isset($coll->full_name) ? ' - '. $coll->full_name : '' }} {{isset($coll->name) && $coll->name != $coll->$name_key ? ' - '. $coll->name : '' }}</option>
                    @endif
                ...
                @endphp
                    ...
                    @endif --}}
                    <option value="{{$coll->$id}}">{{$coll->$name}} {{isset($coll->full_name) ? ' - '. $coll->full_name : '' }} {{isset($coll->name) && $coll->$name != $coll->name ? ' - '. $coll->name : '' }}</option>
                @endif
